...
export async function createVoucher(
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const [salesAccount] = await tx
      ...
      .from(accounts)
      .where(and(eq(accounts.companyId, company.id), eq(accounts.code, "SALES")))

    const [purchaseAccount] = await tx
      .select({ id: accounts.id })
      .from(accounts)
      .where(
        and(eq(accounts.companyId, company.id), eq(accounts.code, "PURCHASE"))
      )
    ...
    if (isItemBased) {
      ...
      for (const line of itemLines) {
        ...
        await tx
          ...
          .where(and(eq(items.id, line.itemId), eq(items.companyId, company.id)))
      }
    }
...
export async function cancelVoucher(companySlug: string, voucherId: string) {
  ...
  await db.transaction(async (tx) => {
    const [voucher] = await tx
      ...
      .from(vouchers)
      .where(
        and(eq(vouchers.id, voucherId), eq(vouchers.companyId, company.id))
      )
    ...
    for (const item of stockItems) {
      ...
      await tx
        ...
        .where(
          and(eq(items.id, item.itemId), eq(items.companyId, company.id))
        )
    }
